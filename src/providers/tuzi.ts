...
export const tuziProvider: Provider = {
  ...
  async generateImage(params: ImageParams): Promise<ImageResult> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ProviderError(`Tuzi API error: ${error.error?.message || response.statusText}`, 'tuzi', response.status);
      }
      ...
      return { url, outputPath, metadata: { provider: 'tuzi', model, size: requestBody.size } };
    } catch (error) {
  ...
  async editImage(params: EditImageParams): Promise<ImageResult> {
    ...
    try {
      ...
      return { url, outputPath, metadata: { provider: 'tuzi', model } };
    } catch (error) {
      if (error instanceof ProviderError) throw error;
      throw new NetworkError(`Tuzi API request failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  async batchGenerateImages(params: BatchImageParams): Promise<BatchImageResult[]> {
    ...
    if (!existsSync(params.batchFile)) {
      throw new ProviderError(`Batch file not found: ${params.batchFile}`, 'tuzi');
    }
    
    const batchContent = readFileSync(params.batchFile, 'utf-8');
    const batchItems = JSON.parse(batchContent);
    if (!Array.isArray(batchItems)) {
      throw new ProviderError('Batch file must contain a JSON array', 'tuzi');
    }
